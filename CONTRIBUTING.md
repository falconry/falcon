...
Large language models (LLM) for coding are getting increasingly sophisticated,
and they *might* be useful for contributing to Falcon too. However, at the same
time we are trying to avoid AI-generated "workslop", so if you do rely on an LLM
to generate parts of your changeset, please carefully review and test the code
manually before submitting the PR.

In addition, if a substantial part of your pull request has been generated by
an LLM, you are expected to disclose this in your PR's description or comments.
Pull requests that show signs of unreviewed or undisclosed LLM output may be
closed without detailed review. Relatedly, please do not credit LLMs or coding
agents as commit authors (e.g., via `Co-authored-by` trailers that some tools
add by default); disclose their use as described here instead.

Furthermore, autonomous coding agents must never open pull requests on their
own; `AGENTS.md` spells out the boundaries that apply to agent-assisted
