...
class Request(object):
    ...
    Attributes:
        ...
        context (dict): Dictionary to hold any data about the request which is
            ...
            To use a type other than *dict*, you will need to extend this
            class and override the ``_initialize_context`` method, along with
            passing the new type to the ``request_type`` argument of
            ``falcon.API()``.
    ...
    def __init__(self, env):
        ...
        # Initialize the app-specific context container
    ...
    def _initialize_context(self):
        """Initialize self.context to its default dictionary value.
        Developers wishing to use a type other than *dict* (e.g. something
        more efficient generated by ``collections.namedtuple()``) should
        extend the ``Request`` class and override this method.  Don't forget
        to pass the new type to the ``request_type`` argument of
        ``falcon.API()``.
        """
