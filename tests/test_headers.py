...
class UnicodeHeaderResource(HeaderHelpersResource):
    ...
    def on_head(self, req, resp):
        resp.set_header('Content-Type', six.u('x-swallow/unladen'))
        resp.set_header(six.u('X-Auth-Token'), 'setecastronomy')
        resp.set_header(six.u('X-AUTH-TOKEN'), six.u('toomanysecrets'))
