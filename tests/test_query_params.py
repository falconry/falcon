...
class TestQueryParams:
    ...
    def test_get_param_as_media_default_media_type(self, simulate_request, client, resource):
        # Test L2361-2362: media_type=None fallback to default_media_type
        client.app.add_route('/', resource)
    ...
    def test_get_param_as_media_no_handler_json_fallback(self, simulate_request, client, resource):
        # Test L2369, L2372-2373: handler is None but media_type contains JSON
        client.app.add_route('/', resource)
        payload_dict = {'foo': 'bar'}
        query_string = f'payload={json.dumps(payload_dict)}'
        simulate_request(client=client, path='/', query_string=query_string)
        req = resource.captured_req
        # Use a media type that contains 'application/json' substring but has no explicit handler
        result = req.get_param_as_media('payload', media_type='custom/application/json')
        assert result == payload_dict

    def test_get_param_as_media_no_handler_no_json(self, simulate_request, client, resource):
        # Test L2369: handler is None and media_type doesn't contain JSON
        client.app.add_route('/', resource)
